...
\item While on that same module page in localdb. If there is a warning that is telling you to sync the FE Chip configs for the current stage you can do that by clicking the hyperlink displayed in the warning message labeled "sync the FE chip configs".
\end{itemize}
...
\begin{itemize}
    ...
    \item Use a small amount of isopropyl alcohol (IPA) on a cotton tipped applicator that you can find by the sink in the back of the clean room to wipe down the vacuum chuck. You can unplug the ESD strap to go to the sink, just plug it back in before handling any modules.
    \item Make sure that the rubber gasket is flush with the chuck.
    \item Make sure that you are wearing ESD strap now.
    \item Place the module into the white plastic carrier. Unscrew the base plate screws, alternating between the screws, doing half a screw at a time. 
...
The data adapter card has a lever that keeps the pigtail locked in its connection so to connect the pigtail the black lever must be up before inserting the connector and then flipped down to lock it in place. 
    \item Make sure dry air tube is inside the foam, so that is blowing air directly on top of the module and place the foam lid back on 
\begin{figure}[H]
    \centering
...
\newpage
\section{Pretest Checks}
The following things should be checked before doing any tests. 

\begin{itemize}
    \item inerlock status
    \item Meerstetter status
    \item Humidity
    \item Pressure
    \item Chiller temp
\end{itemize}
...
\end{verbatim}
If the IV measurement failed, contact an expert. If IV looks okay, turn on the LV module power, set the HV to 120 V, and turn on the HV power. See Table \ref{tab:commands} for a list of commands that can be run on daq-05.
...
\subsubsection{Pixel Failure Analysis}
...
\begin{itemize}
    \item the module is in the thermal cycling stage in localdb
    \item chiller running below -10$^\circ$C
    \item meerstetter running and stable at 20$^\circ$C
    \item module is powered off
\end{itemize}
...
An example of the module temperature during a good thermal cycling can be seen in Fig \ref{fig:thermalcycle}.

\begin{figure}[H]
    \centering
    \includegraphics[width=.5\linewidth]{Figures/thetmalcyclegood.png}
    \caption{Example of a successful thermal cycling with one extreme cycle and 10 cycles.}
    \label{fig:thermalcycle}
\end{figure}

If everything looks okay it can be uploaded to localdb using the manual input feature in the thermal cycling stage for that module. An example of how this was uploaded is shown in Fig \ref{fig:thermal-cycle-manual-input} where the start date/time and end date/time are determined from the grafana thermal cycling state variable mentioned earlier. 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Figures/theramal_cycle_data_input.png}
    \caption{Example of manual data input for thermal cycling. }
    \label{fig:thermal-cycle-manual-input}
\end{figure}
...
To reset the meerstetter follow these steps, the commands for each step can be found in Table \ref{tab:commands}. 
\begin{itemize}
    \item if the meerstetter is still running, disable it, otherwise continue to next step
    \item reset the meerstetter
    \item initiate the meerstetter
\end{itemize}
